...
func getPlatformProperties() map[string]string {
	return map[string]string{
		...
		"X-Qoder-Runtime":         "go",
		"X-Qoder-Runtime-Version": runtime.Version(),
	}
}
...
func NewRequestConfig(ctx context.Context, method string, u string, body any, dst any, opts ...RequestOption) (*RequestConfig, error) {
	...
	req.Header.Set("X-Qoder-Retry-Count", "0")
	req.Header.Set("X-Qoder-Timeout", "0")
	...
	if req.Header.Get("X-Qoder-Timeout") == "0" {
		if cfg.RequestTimeout == time.Duration(0) {
			req.Header.Del("X-Qoder-Timeout")
		} else {
			req.Header.Set("X-Qoder-Timeout", strconv.Itoa(int(cfg.RequestTimeout.Seconds())))
		}
	}
...
func shouldRetry(req *http.Request, res *http.Response) bool {
	...
	if req.Method != http.MethodGet && req.Method != http.MethodHead && req.Header.Get("Idempotency-Key") == "" && (res == nil || res.StatusCode != http.StatusTooManyRequests) {
		return false
	}
...
func (cfg *RequestConfig) Execute() (err error) {
	...
	shouldSendRetryCount := cfg.Request.Header.Get("X-Qoder-Retry-Count") == "0"
	...
	for retryCount := 0; retryCount <= cfg.MaxRetries; retryCount += 1 {
		...
		if shouldSendRetryCount {
			req.Header.Set("X-Qoder-Retry-Count", strconv.Itoa(retryCount))
		}
	...
	if res == nil {
		return fmt.Errorf("qoder: transport returned a nil response")
	}
	if res.Request == nil {
		res.Request = cfg.Request
	}
	if res.StatusCode >= 400 {
		...
		aerr := apierror.Error{Request: cfg.Request, Response: res, StatusCode: res.StatusCode, RequestID: res.Header.Get("x-request-id"), WorkspaceID: ""}
	...
	if cfg.ResponseBodyInto == nil && cfg.ResponseInto == nil {
		// No caller owns this body; release the connection and request timer here.
	...
	if res.StatusCode == http.StatusNoContent || len(bytes.TrimSpace(contents)) == 0 {
		return nil
	}
	// If we are not json, return plaintext
	contentType := res.Header.Get("content-type")
	mediaType, _, _ := mime.ParseMediaType(contentType)
	isJSON := (contentType == "" && json.Valid(contents)) || strings.Contains(mediaType, "application/json") || strings.HasSuffix(mediaType, "+json")
	if !isJSON {
