...
func (r *Page[T]) GetNextPage() (res *Page[T], err error) {
	...
	if r.cfg == nil {
		return nil, fmt.Errorf("page is not attached to a client request")
	}
	cfg := r.cfg.Clone(r.cfg.Context)
	if cfg == nil {
		return nil, fmt.Errorf("pagination request body cannot be replayed")
	}
	if r.cfg.Request.URL.Query().Has("before_id") {
		...
		if r.cfg.Request.URL.Query().Get("before_id") == next {
			return nil, fmt.Errorf("pagination cursor did not advance")
		}
		err = cfg.Apply(option.WithQueryDel("after_id"), option.WithQuery("before_id", next))
		if err != nil {
			return nil, err
		}
	} else {
		...
		if r.cfg.Request.URL.Query().Get("after_id") == next {
			return nil, fmt.Errorf("pagination cursor did not advance")
		}
...
func (r *PageCursor[T]) GetNextPage() (res *PageCursor[T], err error) {
	if r.JSON.HasMore.Valid() && !r.HasMore {
		return nil, nil
	}
	next := r.NextPage
	if len(next) == 0 {
	...
	if r.cfg.Request.URL.Query().Get("page") == next {
		return nil, fmt.Errorf("pagination cursor did not advance")
	}
	cfg := r.cfg.Clone(r.cfg.Context)
	if cfg == nil {
	...
	err = cfg.Apply(option.WithQueryDel("before_id"), option.WithQueryDel("after_id"), option.WithQuery("page", next))
	if err != nil {
		return nil, err
	}
	var raw *http.Response
