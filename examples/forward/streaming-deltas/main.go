...
func run(ctx context.Context, r *live.Run, s *forwardutil.Example) error {
	...
	stream := s.Client.Sessions.Events.StreamEvents(ctx, session, forward.SessionEventStreamParams{EventDeltas: []string{"agent.message"}}, option.WithRequestTimeout(s.Config.Timeout))
	defer stream.Close()
