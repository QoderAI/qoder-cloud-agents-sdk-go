...
import (
	"fmt"
	shimjson "github.com/QoderAI/qoder-cloud-agents-sdk-go/convention/encoding/json"
	"io"
	"mime/multipart"
	"net/textproto"
	"path"
		...
		for i := 0; i < r.NumField(); i++ {
			...
			if ptag.metadata {
				encoderFn = func(key string, value reflect.Value, writer *multipart.Writer) error {
					if ptag.omitzero && value.IsZero() {
						return nil
					}
					b, err := shimjson.Marshal(value.Interface())
					if err != nil {
						return err
					}
					return writer.WriteField(key, string(b))
				}
			} else if ptag.omitzero {
...
func (e *encoder) newReaderTypeEncoder() encoderFunc {
	return func(key string, value reflect.Value, writer *multipart.Writer) error {
		...
		if strings.ContainsAny(filename+contentType, "\r\n") {
			return fmt.Errorf("invalid multipart filename or content type")
		}
